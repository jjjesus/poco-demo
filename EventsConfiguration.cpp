...
CEventsConfiguration::MyEvent::MyEvent(DateTime inDate,
...
CEventsConfiguration::CEventsConfiguration(std::istream& istr) :
		IniFileConfiguration(istr) {
	Construct();
}

// Constructor that receive path as std::string
CEventsConfiguration::CEventsConfiguration(const std::string& path) :
...
CEventsConfiguration::~CEventsConfiguration() {
	...
	for (iterator = mMyEvents.begin(); iterator != mMyEvents.end();
			iterator++) {
		if (*iterator != NULL) {
			delete *iterator;
		}
...
void CEventsConfiguration::Construct() {
	...
	CAppLogger::Instance().Log("Reading events configuration file...",
			Message::PRIO_DEBUG);
	...
	while (!quit) {
		try {
			...
			int timeZoneDiff;
			DateTime date;
			if (!DateTimeParser::tryParse("%d/%n/%Y", dateStr, date,
					timeZoneDiff)) {
	...
	// Write to log
	CAppLogger::Instance().Log("Events configuration file read successful",
			Message::PRIO_DEBUG);
	ostringstream msg;
...
CEventsConfiguration::MyEvent CEventsConfiguration::operator [](
		int inIndex) const {
